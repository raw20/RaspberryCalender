...
### 라즈베리파이 내의 시간을 현재시간으로 설정 후 FND에 시간이 출력 할수 있게
13주차때 시계모듈을 구입후 구현을 했으나 시간의 초기화에 오류가 발생해 검색을 한 결과 우리가 구입한 모듈이 DS1602는 더 이상 아두이노에서 지원을 하지않아 정상적으로 작동이 되지 않는다고 하여서 최신모듈로 구입을 하였다. 부품이 도착하면 15주차때 최종 구현할 예정이다.
